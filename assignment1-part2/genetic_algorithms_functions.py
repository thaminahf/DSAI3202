...
def calculate_fitness(route,
    ...
    for i in range(len(route) - 1):
        ...
        # If the distance is 100000, the route is infeasible
        if distance == 100000:
            return -1e6  # Return a large negative penalty
        
        total_distance += distance
    
    # Return the negative of the total distance (since we want to minimize distance)
...
def select_in_tournament(population,
    ...
    for _ in range(number_tournaments):
        ...
        tournament_individuals = [population[i] for i in idx]
        tournament_scores = [scores[i] for i in idx]
        
        # Find the individual with the highest fitness score in the tournament
        best_idx = idx[np.argmax(tournament_scores)]
...
def mutate(route,
    ...
    if np.random.rand() < mutation_rate:
        ...
        # Check if the mutated route is feasible
        feasible = True
        for k in range(len(route) - 1):
            if distance_matrix[route[k]][route[k + 1]] == 100000:
                feasible = False
                break
        
        # If the route is infeasible, revert the mutation
        if not feasible:
            route[i], route[j] = route[j], route[i]
...
def generate_unique_population(population_size, num_nodes,distance_matrix):
    ...
    while len(population) < population_size:
        ...
        for i in range(len(individual) - 1):
            if distance_matrix[individual[i]][individual[i + 1]] == 100000:
                feasible = False
                break
